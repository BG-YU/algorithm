...
int solution(int bridge_length, int weight, vector<int> truck_weights) {
    ...
    vector<int> used(truck_weights.size(), 0);

    while (1) {
        answer++;
        for (int k = 0; k < vect.size(); k++) {
            ++vect[k].second;
            if (vect[k].second == bridge_length) {
                sum -= vect[k].first;
        ...
        if (i < truck_weights.size() && sum + truck_weights[i] <= weight && used[i] == 0) {
            vect.push_back({ truck_weights[i], 0 });
            used[i] = 1;
            sum += truck_weights[i];
            i++;
        }
