...
string fn_set(string msg, string str, int index) {
    if (um.count(str) <= 0) {
        um[str] = ++maxx;
        return str.substr(0, str.length() - 1);
    }

    return fn_set(msg, str + msg[index], index + 1);
}
...
vector<int> solution(string msg) {
    ...
    for (int i = 0; i < msg.length(); i++) {
        string str = "";
        str += msg[i];
        string tmp = fn_set(msg, str, i + 1);
        for (int k = 0; k < tmp.length() - 1; k++) i++;
        answer.push_back(um[tmp]);
    }
    return answer;
}
