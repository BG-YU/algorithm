...
vector<Node> vect = {{"abc", "10", ".txt"}, {"abc", "10", ".jpg"}};

위 vect를 정렬한다고 할때
1. head는 사전순으로
2. number은 오름차순으로 ex) 1 -> 2 -> 3
3. head와 number이 같을 때는 입력된 순서, 즉 tail은 정렬기준에 포함되지 않는다.

불안정 정렬을 하게 되면 값은 동일하지만 아래와 같이 동일한 값이 앞에 올지 뒤에 올지 알수 없다.
정렬기준에 포함되지 않은 tail의 값이 앞에 올지 뒤에 올지 알수 없다.
...
안정정렬은 동일한 값이 항상 동일하게 표시가 된다.
정렬기준에 포함되지 않은 tail의 값이 항상 동일하게 표시된다.
...
Node fn_split(string str) {
    ...
    if (end - st > 5) end = st + 5;

    node.head = str.substr(0, st);
    node.number = str.substr(st, end - st);
    node.tail = str.substr(end, str.length() - end);
...
bool cmp(Node a, Node b) {
    string str1 = fn_upper(a.head);
    string str2 = fn_upper(b.head);
...
vector<string> solution(vector<string> files) {
    ...
    for (int i = 0; i < vect.size(); i++) {
        answer.push_back(vect[i].head + vect[i].number + vect[i].tail);
    }
