...
이를 이용하여 문자열 사이의 유사도를 계산하는데 이용할 수 있다. 문자열 FRANCE와 FRENCH가 주어졌을 때,
...
기타 공백이나 숫자, 특수 문자가 들어있는 경우는 그 글자 쌍을 버린다. 예를 들어 ab+가 입력으로 들어오면, ab만 다중집합의 원소로 삼고, b+는 버린다.
...
void set_hashmap(string str, unordered_map<string, int> &um) {
    string temp = "";
    int flag = 0;
    ...
    for (int i = 0; i < str.length(); i++) {
        if (str[i] >= 'a' && str[i] <= 'z') temp += str[i];
            ...
            if (str[i] >= 'A' && str[i] <= 'Z') temp += str[i] + ('a' - 'A');
            else {
                temp += str[i];
                flag = 1;
            }
...
int solution(string str1, string str2) {
    ...
    set_hashmap(str1, um1);
    set_hashmap(str2, um2);
    
    if (um1.size() == 0 || um2.size() == 0) return 65536;

    for (auto elem : um1) {
        for (int i = 0; i < elem.second; i++) {
            if (um2.count(elem.first) > 0 && um2[elem.first] > 0) {
                gyo++;
                um1[elem.first]--;
                um2[elem.first]--;
            }
    ...
    hap -= gyo;
    temp = to_string((gyo / hap) * 65536);
    index = temp.find('.', 0);
    if (index < 0) answer = stoi(temp);
    else answer = stoi(temp.substr(0, index));
